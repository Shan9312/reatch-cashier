  ...
  <div class="keyboard-page">
    <div class="content">
      ...
      <div class="password-list">
        <span
          :class="{'password-point': item <= verificationCodeArr.length}"
          v-for="item in 6"
          :key="item"
        ></span>
      </div>
      ...
      <div
        class="pay-btn"
        :class="{'text-color-red':verificationCodeArr.length === 6}"
        @click="handlePayBtn"
      >确认支付</div>
      <!-- 短信提示信息 -->
      ...
      <ul class="keyboard-num">
        <li
          v-for="(item,index) in keyboardNumArr"
          :key="index"
          @click="handleEntryNum(item)"
        >{{item}}</li>
        <li>
          <img
...
export default {
  ...
  methods: {
     ...
     * */
    handlePayBtn() {
      if (!this.verificationCodeArr || this.verificationCodeArr.length != 6)
        return;
      this.$emit("handlePayOrderBtn", this.verificationCodeArr.join(""));
    },
    /**
     * 手动输入密码
     *
     ...
     * */
    handleDelNum() {
      if (this.verificationCodeArr && this.verificationCodeArr.length) {
    ...
    handleForgetPassword() {
      dooolyAPP.gotoJumpJq(
        this.$router,
        `${GlobalProperty.frontendDomain.m}verification/0/2`
      );
    },
    /**
     * 若是积分付款则 60s倒计时
