...
const Http = (browserName, getUrlParams) => {
  ...
  axios.interceptors.request.use(
    config => {
      ...
      if (localStorage.groupId || Cookies.get('groupId')) {
        config.headers.groupId = localStorage.groupId || Cookies.get('groupId')
      }
      if (browserName === 'WeChat') {
        ...
        let orderSource = getUrlParams('orderSource')
        // 免登录，不需要传channel
        if (orderSource == 'meituan') {
          localStorage.appUrlChannel = ''
        } else {
